...
namespace LiteCommerce.Common
{
    public static class ViewHelper
    {
        ...
        public static IHtmlContent RenderPageButtonToRight(string ControllerName, int CurrentPage, int Width, string SearchValue)
        {
            var html = new HtmlContentBuilder();
            for (int i = CurrentPage; i <= Width; i++)
                html.AppendHtml(RenderButton(ControllerName, i, SearchValue, Convert.ToString(i)));

            return html;
        }
        public static IHtmlContent RenderPageButtonToLeft(string ControllerName, int CurrentPage, int Width, string SearchValue)
        {
            var html = new HtmlContentBuilder();
        ...
        public static IHtmlContent RenderPageButton(string ControllerName, int CurrentPage, int MaxPage, int Width, string SearchValue)
        {
            var html = new HtmlContentBuilder();
            ...
            if (MaxWidthLeft > 1)
            {
                html.AppendHtml(RenderButton(ControllerName, 1, SearchValue, "«"));
                html.AppendHtml("<li><a>...</a></li>");
            }

            // Page is first page => | 1 | 2 | 3 | 4 |...| » |
            if (CurrentPage == 1)
                html.AppendHtml(RenderPageButtonToRight(ControllerName, CurrentPage, MaxWidthRight, SearchValue));

            // Page is last page => |n-4|n-3|n-2|n-1| n |
            else if (CurrentPage == MaxPage)
                html.AppendHtml(RenderPageButtonToLeft(ControllerName, CurrentPage, MaxWidthLeft, SearchValue));

            // Page is between first and last page => |n-3|n-2|n-1| n |n+1|n+2|n+3|
            else
            {
                html.AppendHtml(RenderPageButtonToLeft(ControllerName, CurrentPage - 1, MaxWidthLeft, SearchValue));
                html.AppendHtml($"<li class='active'><a>{CurrentPage}</a></li>");
                html.AppendHtml(RenderPageButtonToRight(ControllerName, CurrentPage + 1, MaxWidthRight, SearchValue));
            }
            ...
            {
                html.AppendHtml("<li><a>...</a></li>");
                html.AppendHtml(RenderButton(ControllerName, MaxPage, SearchValue, "»"));
            }
